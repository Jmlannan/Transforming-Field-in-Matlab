...
for idx = 1:numel(X)
    ...
    elementy = Y(idx);
    elementz = Z(idx);
%    vec = coulomb(elementx,elementy,elementz,10);
    vec = [1,1,1] %constant field, ie a capacitor
    U(idx) = vec(1);
    V(idx) = vec(2);
    W(idx) = vec(3);
end
